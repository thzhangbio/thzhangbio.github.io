---
title: 测试文章  # 文章标题，显示在页面标题、文章列表和浏览器标签页
date: 2025-11-03  # 发布日期，格式为 YYYY-MM-DD，用于排序和显示
last_modified_at: 2025-11-04  # 最后修改日期（可选），如果设置会在页面上显示"最后更新"时间
categories: [教程, 示例]  # 文章分类数组，可以设置多个分类，用于分类归档页面
tags: [Jekyll, Chirpy, 示例, 教程]  # 文章标签数组，可以设置多个标签，用于标签归档页面和标签云
author: 天5# 作者名称（可选），如果不设置则使用 _config.yml 中的 site.social.name
...
| 静轩之别苑 | [http://quickapp.lovejade.cn](https://quickapp.lovejade.cn/?ref=markdown.lovejade.cn) |2019-01-12|
